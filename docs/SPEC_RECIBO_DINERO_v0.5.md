...
- **Interfaz de usuario** (`apps/web-operacion`): formulario, detalle, listado y pagina publica de `RecibosView`/`ReciboPublicaView` **no se tocan** en esta iteracion — quedan consumiendo el mismo contrato de respuesta (`ReciboRecord` con los campos que ya usan), y seguiran funcionando porque no se eliminan campos que el frontend ya lee (`pagador_nombre`, `concepto`, `importe`, `forma_pago`, `verification_token`, etc.). Los campos nuevos (`moneda`, `xml_hash`, `pdf_hash`, `anulacion_motivo`, estado `ANULADO`, boton "Anular", descarga de XML) **no se exponen en la UI todavia** — se documentan para una iteracion `v0.6` (UI) posterior.
