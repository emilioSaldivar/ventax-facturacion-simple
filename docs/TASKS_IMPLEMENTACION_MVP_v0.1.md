...
| FE-005 | FiscalGateway | Usar `envio.mode=BATCH` por defecto en facturadores | DONE | FE-001, INV-009 | Facturas y NCE operativas llaman a FE con `envio.mode=BATCH`; `SYNC` queda solo para smoke opt-in/desarrollo |
| FE-006 | FiscalGateway | Persistir diagnostico de lote retornado por FE | DONE | FE-005 | Se guarda `delivery_mode`, `batch_id`, `dProtConsLote`, codigos/mensajes de recepcion/consulta y estado resumido sin exponer payload fiscal crudo |
...
| INV-012 | Facturacion resiliente | Ajustar estados SaaS para procesamiento por lote | DONE | FE-005, FE-006 | Respuesta batch recibida/pending queda como `PENDIENTE_SIFEN` con diagnostico y accion `refresh-status`; aprobado/rechazado posterior actualiza estado operativo |
| INV-013 | Facturacion resiliente | Consultar seguimiento de lote/estado desde FE | DONE | INV-012 | `refresh-status` usa consulta por CDC y preserva diagnostico batch previo; no consulta SIFEN directo desde frontend |
...
| QA-010 | QA | Cubrir emision batch por defecto | DONE | FE-005, INV-012 | Tests unitarios/integracion verifican payload `envio.mode=BATCH`, persistencia de batch y transicion a `PENDIENTE_SIFEN`; smoke opt-in queda sin versionar secretos |
...
- 2026-05-23: agregado selector de `tipo de servicio` en `Nueva factura` con opciones `1` venta de mercaderia, `2` prestacion de servicios (default) y `3` mixto; el frontend envia `tipo_transaccion` en preview/emision, API valida `1|2|3` (default backend `2` por compatibilidad) y FiscalGateway lo delega como `tipoTransaccion` en `POST /factura` hacia `facturacion-electronica`. Actualizados `spec/openapi.yaml` y cadena SDD `DOCUMENTOS_EMISION_UX` (SPEC/PLAN/TASKS). Verificacion ejecutada: `npm run test --workspace @facturacion-simple/api`, `npm run typecheck --workspace @facturacion-simple/api`, `npm run lint --workspace @facturacion-simple/api`, `npm run typecheck --workspace @facturacion-simple/web-operacion` y `npm run build --workspace @facturacion-simple/web-operacion`.
...
- 2026-05-22: planificada migracion de emision fiscal SaaS a envio SIFEN por lotes por defecto, alineada con `contexto_facturacion_electronica/docs_base/openapi.yaml`: FE acepta `envio.mode=BATCH`, retorna `delivery_mode` y `batch`, y expone cola/lotes pendientes. Se actualizan SPEC, PLAN, TASKS y `spec/openapi.yaml`; quedan PENDING `FE-005`, `FE-006`, `INV-012`, `INV-013` y `QA-010`. Validacion documental: revision de contrato FE y OpenAPI SaaS sin ejecutar implementacion.
- 2026-05-23: cerradas `FE-005`, `FE-006`, `INV-012`, `INV-013` y `QA-010`. `RealFiscalGateway` envia facturas y NCE con `envio.mode=BATCH`, normaliza `delivery_mode`/`batch`, expone `fiscal_envio_modo` y `batch` en `DocumentoResponse`, conserva diagnostico batch al refrescar por CDC y mantiene `PENDIENTE_SIFEN` para respuestas batch no finales. Validacion ejecutada: `npm run typecheck --workspace @facturacion-simple/api`, `npm run typecheck --workspace @facturacion-simple/web-operacion`, `npx vitest run apps/api/tests/fiscal-gateway.test.ts apps/api/tests/facturas.service.test.ts`, `npm run test --workspace @facturacion-simple/api`, `npm run lint`, `npm run build`, `npm run qa:no-secrets`, parseo YAML de `spec/openapi.yaml`, `bash scripts/deploy.sh` con build de imagenes; el primer deploy quedo bloqueado por `FE_DOCKER_NETWORK=bridge` y `DATABASE_URL` local de host dentro de contenedor, luego se valido stack con `FE_DOCKER_NETWORK=facturacion-electronica_default` y `DATABASE_URL=postgres://facturacion_simple:facturacion_simple@postgres:5432/facturacion_simple`: `api`, `frontend` y `postgres` healthy, `/healthz` y `/api/v1/health` OK.
- 2026-05-24: refinamiento UX operativo implementado para `Nueva factura` e `Informacion` (cadena `RUX`): readiness en lenguaje de negocio (`Membresia activa`, `Suscripcion al dia`, `Facturador activo`, `Configuracion fiscal completa`), cabecera fiscal colapsable por icono de ojo, eliminacion de nota de numero fiscal pendiente, placeholder de documento `Ingrese numero de documento`, teclado numerico condicional para `RUC`/`CI`, marcacion de obligatorios/opcionales, modal de producto con `Cantidad` primero y foco en `Descripcion`, selector tactil de IVA (`5%`, `10%`, `EX`) y reemplazo de `Opciones avanzadas` por `+ Agregar codigo interno`, con decision guardar/no guardar item en catalogo y salto al bloque de envio tras confirmar linea. Validacion local: `npm run typecheck --workspace @facturacion-simple/api`, `npm run typecheck --workspace @facturacion-simple/web-operacion`, `npx vitest run apps/api/tests/fiscal-gateway.test.ts`, `npm run test --workspace @facturacion-simple/api`, `npm run build --workspace @facturacion-simple/web-operacion`, `npm run qa:no-secrets`, `APP_ENV_FILE=/tmp/.env.testing FE_DOCKER_NETWORK=fe_local bash scripts/deploy.sh`, healthchecks `8092/8092/api/v1/health` y `APP_ENV_FILE=/tmp/.env.testing ONBOARDING_SMOKE_NCE=YES npm run ops:onboarding-smoke` exitoso (`smokeId onboarding-20260524031919293`, KUDE/XML OK, NCE `EMITIDA`).
