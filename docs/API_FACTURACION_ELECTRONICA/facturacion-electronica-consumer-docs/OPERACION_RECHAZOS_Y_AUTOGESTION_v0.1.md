...
- Identificacion del documento en formato fiscal (establecimiento, punto y numero de factura).
- Actividad economica asociada al documento.
- Motivo de rechazo reportado por SIFEN (codigo y descripcion cuando exista).
- Fecha de emision y estado de ventana operativa (dentro/fuera de 72h).
...
## 5.1 Alerta por inutilización registrada

Al registrar un evento de inutilización de numeración, el sistema intenta enviar correo al email configurado del facturador con:

- RUC,
- establecimiento y punto de expedición,
- rango inutilizado,
- motivo,
- id y estado actual del evento.

La notificación es no bloqueante: si no existe correo o falla SMTP, el evento fiscal igualmente queda registrado.
...
## 10.1 Notificación por cancelación al cliente final

Cuando se solicita una cancelación fiscal y el evento resulta `ACCEPTED`, el sistema intenta notificar al cliente final si el documento tiene email del receptor:

- asunto: anulación de factura electrónica,
- incluye motivo de anulación,
- adjunta XML y KUDE PDF del documento anulado.

Si no existe email o faltan artefactos, la cancelación fiscal no se bloquea; la respuesta devuelve estado de notificación para seguimiento operativo.

## 10.2 Taxonomía operativa para cancelar vs anular

- `cancelar`: retirar una FE de `QUEUED_BATCH` antes de transmitirla a SIFEN. No genera evento fiscal.
- `anular`: registrar evento fiscal de cancelación para un DTE ya aprobado en SIFEN, dentro de la ventana aplicable.
- `NCE_REQUIRED`: cuando la ventana de anulación expira, el backend responde que corresponde emitir una `NCE`; no la ejecuta automáticamente.

Si luego de una cancelación local se corrige el body de la factura:
- puede mantenerse el mismo `CDC` solo si la prevalidación indica `CDC_NO_CHANGE`;
- si la corrección altera la base fiscal, debe informarse `CDC_CHANGE` y seguir el flujo de corrección correspondiente.
...
## 11.1 Paso a paso retroactivo (sin full scan)

Cuando exista sospecha de rechazo sin alerta o batch pendiente que quedo `fresh`:

1. Ejecutar `Lanzar sync` para el emisor/ambiente con limite acotado (ej. 20/50).
2. Mantener `only_stale=true` (perfil operativo normal).
3. El backend ejecuta pasada incremental interna para:
- `QUEUED_BATCH` y `SENT_BATCH`,
- `REJECTED` sin alerta `ALERT` exitosa.
4. Verificar en trazas:
- `de_status_sync_job_items` con `document_id` esperado,
- `de_send_attempts` con `mode=ALERT` (success true/false y motivo).
5. Si un caso puntual sigue pendiente, relanzar sync selectivo por `document_ids` o `cdcs`.
