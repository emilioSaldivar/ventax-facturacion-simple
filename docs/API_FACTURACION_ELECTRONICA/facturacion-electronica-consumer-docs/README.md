...
> `facturacion-electronica-consumer-docs` (paquete filtrado para consumidores, base `/v1`) para
> respaldar `SPEC_RECIBO_DINERO_v0.5.md` (Recibos de Dinero Firmados Digitalmente, sección 16 de
> la guía). El `openapi.yaml` (sin sufijo, 213 KB) y los `OPERACION_*.md` restantes en esta carpeta
> son una vendorización previa (jun-2026, base `/fcws`, incluye superficie `/admin/*`) que
> **se conserva sin tocar** porque varios SPEC/PLAN/TASKS propios de este repo
> (`SPEC_BACKOFFICE_ADMIN_v0.1.md`, `SPEC_AUTOGESTION_AVANZADA_SOPORTE_v0.1.md`,
> `SPEC_CONTRATO_CANONICO_FISCAL_v0.1.md`, entre otros) la referencian directamente. Producción
> (`factura.ventax.app`) sigue consumiendo la API fiscal por `/fcws` (ver memoria
> `fiscal-gateway-staging-v1-migration`); staging ya usa `/v1`. No asumir que los ejemplos con
> `/v1` de este paquete filtrado aplican tal cual a producción sin verificar el `APP_ENV_FILE`
> vigente.
...
- `GET /v1/verificar/recibo/{token}` y `/pdf` — verificación pública de un recibo, sin autenticación.
