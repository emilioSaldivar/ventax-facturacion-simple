...
- 2026-06-04: CCF-002 CERRADO — Backfill ejecutado en staging. CSV generado manualmente desde pgAdmin exportando `SELECT cdc, document_uuid FROM de_documents WHERE cdc IS NOT NULL AND document_uuid IS NOT NULL`. Resultado del backfill: total=39, con_uuid=23, sin_uuid_con_cdc=0 (el 1 restante investigado y confirmado sin CDC real), sin_uuid_sin_cdc=15 (documentos sin CDC — borradores o errores de emisión, esperado). CCF-023 — Backfill en producción PENDIENTE, ejecutar con el mismo CSV y flujo en ventana de silencio.
- 2026-06-04: CCF-021 y CCF-022 — PENDIENTE validación sobre contenedores (deploy) y Playwright. Requieren smoke post-deploy en staging.
